...
def make_primers(cur_record):
    ...
    primer_cl += "-osize 20 "
    primer_cl += "-minsize 18 "
    primer_cl += "-maxsize 27 "
    ...
    primer_cl += "-maxpolyx 3 "

    primer_cl += "-osizeopt 22 "
    primer_cl += "-ominsize 18 "
    primer_cl += "-omaxsize 27 "
...
#        return (False, False)
    if probeseq[:2].count('G') == 0 and probeseq[-2:].count('C') == 0:
        ...
        if probeseq.count('G') >= probeseq.count('C'):
            return (reverse_complement(probeseq), 'minus')
    return (False, False)
